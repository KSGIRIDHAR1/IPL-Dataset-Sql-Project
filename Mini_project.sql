...
select * from ipl_bidder_points;
select * from ipl_bidding_details where bidder_id=103;

select bp.bidder_id,count(if(bid_status='Won',1,NULL))/no_of_bids*100 as won
from ipl_bidding_details bd inner join ipl_bidder_points bp
on bp.bidder_id = bd.bidder_id
group by bidder_id having won>0;

-- 2.	Display the number of matches conducted at each stadium with stadium name, city from the database.
select s.stadium_id,s.stadium_name,s.city,count(ms.stadium_id) from ipl_stadium s
inner join ipl_match_schedule ms on ms.stadium_id = s.stadium_id
group by s.stadium_id,s.stadium_name,s.city
order by count(ms.stadium_id) desc;

-- 3.	In a given stadium, what is the percentage of wins by a team which has won the toss?

-- select stadium_id,if(toss_winner=2,team_id2,team_id1) as won,count(*) from ipl_match im inner join
-- ipl_match_schedule ims on ims.match_id=im.match_id
-- where toss_winner = match_winner group by stadium_id,won;
-- select * from ipl_match;
-- select * from ipl_match_schedule;
-- select * from ipl_stadium;
-- select *,if(toss_winner=2,team_id2,team_id1) as win_id from ipl_match 
-- where toss_winner = match_winner;

-- select stadium_name,if(toss_winner=2,team_id2,team_id1) as win_id
-- from ipl_match m inner join ipl_match_schedule ms
-- on m.match_id = ms.match_id
-- inner join ipl_stadium s
-- on s.stadium_id = ms.stadium_id
-- where toss_winner = match_winner;

create table match_details (select a.match_id, a.toss_winner, a.match_winner, b.stadium_id from ipl_match a 
inner join ipl_match_schedule b 
where a.match_id = b.match_id);
create table tw_mw(select sum(if(toss_winner = match_winner, 1,0)) as twmw, sum(if(toss_winner != match_winner, 1,0)) as tlmw, a.stadium_id,
(sum(if(toss_winner = match_winner, 1,0))+sum(if(toss_winner != match_winner, 1,0))) as total, b.stadium_name
 from match_details a inner join ipl_stadium b 
 where a.stadium_id = b.stadium_id
group by stadium_id);
select (twmw/total)*100 as percent, stadium_name from tw_mw;
-- 4.	Show the total bids along with bid team and team name.
select bid_team,team_name,count(*) 'total bids'
from ipl_bidding_details db
inner join ipl_team it on it.team_id = db.bid_team 
group by bid_team order by count(*) desc;

-- 5.	Show the team id who won the match as per the win details.
select if(match_winner=1,team_id1,team_id2) team_id,win_details 
from ipl_match;
select * from ipl_match;
-- 6.	Display total matches played, total matches won and total matches lost by team along with its team name.
select team_name,tournmt_id,matches_played,matches_won,matches_lost from ipl_team_standings ts
inner join ipl_team t on t.team_id=ts.team_id;

-- 7.	Display the bowlers for Mumbai Indians team.
select player_name from ipl_team_players tp inner join ipl_player p on tp.player_id = p.player_id where player_role = 'bowler'
and team_id = (select team_id from ipl_team where team_name = 'mumbai indians');

-- 8.	How many all-rounders are there in each team, Display the teams with more than 4 
select team_name,count(*) as 'number of players' from ipl_team_players tp
inner join ipl_team t on t.team_id = tp.team_id
